...
(defun find-executables ()
  ...
  (let ((executables '()))
    (dolist (dir '("/usr/bin" "/usr/local/bin"))
...
(defun connect-to-wifi ()
  "List and connect to available Wi-Fi networks using nmcli."
  ;; nmcli -tf SSID dev wifi list | uniq
  ;; `call-process' does not support piping, so we use Elisp to handle `uniq' functionality.
  ;; Connect to WiFi by piping a password to nmcli --ask.
  (interactive)
  (require 'notifications)
  (let* ((networks (split-string (shell-command-to-string "nmcli -t -f SSID device wifi list") "\n" t))
         (selected (completing-read "Select WiFi network: " networks)))
    (when selected
      (let* ((password (read-passwd (format "Password for %s: " selected)))
             (buffer (get-buffer-create "*nmcli-connect*")))
        (with-current-buffer buffer
          (erase-buffer)
          (let ((proc (make-process
                       :name "nmcli-connect"
                       :buffer buffer
                       :command (list "nmcli" "--ask" "device" "wifi" "connect" selected)
                       :noquery t
                                   ...
                                   (when (string= event "finished\n")
                                     (if (zerop (process-exit-status proc))
                                         (notifications-notify
                                          ...
                                          :body (format "Connected to '%s'" selected)
                                          :app-name "GNU Emacs 30.2"
                                          :icon "emacs"
                                          :urgency 'normal)
                                       (notifications-notify
                                        :title "Connection Failed"
                                        :body (format "Failed to connect to '%s'" selected)
					:app-name "GNU Emacs 30.2"
					:icon "emacs"
					:urgency 'critical)))))))
  ...
  (let ((frame-properties '((name . "emacs-prompt")
    ...
    (with-selected-frame
      ...
      (unwind-protect
	   ...
	   ((eq action 'wifi) (connect-to-wifi))
	   ((eq action 'bluetooth) (connect-to-bluetooth))
...
(defun emacs-run-wifi ()
  (interactive)
  (emacs-run-action 'wifi))
#+END_SRC
** Minibuffer tweaks
#+BEGIN_SRC emacs-lisp
